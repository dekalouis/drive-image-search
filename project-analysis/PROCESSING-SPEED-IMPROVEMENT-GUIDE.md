...
**Key insight:** Gemini 2.0 Flash Lite internally resizes images before processing. Sending a 12 MB photo vs a 1024px-wide version produces near-identical captions, but the 12 MB version costs 10x more base64 encoding and API upload overhead. **Note:** Download time is unchanged (full image must be downloaded first), but encoding/upload time is significantly reduced.
...
**Problem:** Full-resolution images (5-15 MB) are base64-encoded and sent to the API. Base64 encoding inflates size by ~33%. A 10 MB image becomes a 13.3 MB string in the API payload. Gemini's vision models internally resize anyway -- you're paying network and encoding cost for pixels the model discards.

**Solution:** After downloading the full image (required for storage/display), resize it to a max dimension of 1024px (or 1536px if higher fidelity is needed) BEFORE base64-encoding and sending to Gemini.

**Important clarification:** Download time is **unchanged** -- you still need to download the full image from Google Drive first. The resize happens in-memory after download.

**Expected gains:**
- Base64 encoding time: reduced 60-80% (smaller buffer: 100-300 KB vs 5-15 MB)
- API upload payload size: reduced 60-80%
- API upload time: reduced 60-80% (smaller payload = faster upload)
- Caption quality: negligible difference -- Gemini's image understanding operates on resized inputs internally
- **Note:** Resize operation adds ~50-200ms, but this is offset by encoding/upload savings

**Implementation notes:**
- Use `sharp` (add as dependency if not present) to resize the buffer in-memory after download
...
**Realistic improvement: 15-25% faster per image** (savings on encoding/upload, but download time unchanged)
...
|--------|------------------|-----------------------|--------|
| Resize images before Gemini | 15-25% faster | Negligible loss | Low |
...
| Pipeline processing | 5-10% faster | No change | Medium |
...
**Combined realistic improvement: 30-50% reduction in per-image processing time**, bringing throughput from ~12-30 images/minute to ~20-45 images/minute with the same infrastructure.

**Note:** These improvements are not perfectly additive due to overlapping bottlenecks. The 30-50% range accounts for realistic compounding effects. Download time (500-3000ms) remains the largest variable and cannot be optimized.
